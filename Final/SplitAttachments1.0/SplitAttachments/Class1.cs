...
namespace Consero.Plugins.SplitAttachments
{
      ...
      //  [System.Security.Permissions.PermissionSet(System.Security.Permissions.SecurityAction.Assert, Unrestricted = true)]
        ...
        public CBugDisplayDialogItem[] BugDisplayViewLeft(CBug[] rgbug, bool fPublic)
        {
            ...
            * the raw page, display it using the Notifications API */
            ...
            if (rgbug[0].ixStatus != 20 )

            { 
                if (rgbug[0].ixStatus != 69 )
                ...
                new CBugDisplayDialogItem("SplitAttachments", EditableTable(rgbug[0].ixBug).RenderHtml())
        ...
        public string RawPageDisplay()
        {
              ...
              //  int iCopies = 0;
                    ...
                    !api.Security.ValidateActionToken(api.Request[api.AddPluginPrefix("actionToken")].ToString()))
                {
                    ...
                    sError = string.Format("{0}={1}",
                                ...
                                HttpUtility.UrlEncode("SplitAttachments: attachments not splitted because action token was invalid or missing.")
                                );
                }
                else
                {
               ...
               //     for (int i = 0; i < iCopies; i++)
                    {
                        ...
                        newbug.sTitle = bug.sTitle + "- Split" +(j + 1).ToString()+"(of Original case" + bug.ixBug +")";
            ...
            {
                api.Notifications.AddAdminNotification(e.ToString(), "Error while Splitting cases");
            }
        ...
        protected CEditableTable EditableTable(int ixBug)
        {
           ...
           // api.Notifications.AddMessage("calling editable table");
            ...
            CEditableTable editableTable = new CEditableTable("SplitAttachmentTable");
            sTableId = editableTable.sId;
            /* Define the header row of the table */
            editableTable.Header.AddCell("Split Case for each attachment");
            ...
            row.AddCell(HttpUtility.HtmlEncode("If the case has more than one Attachment"));
        ...
        protected CDialogTemplate DialogTemplateNew(int ixBug)
        {
            ...
            dlgTemplateNew.Template.sTitle = "Are you sure to Split the attachments for case " + ixBug.ToString();
